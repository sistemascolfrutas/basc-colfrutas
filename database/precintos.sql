...
create table if not exists public.precintos_asignaciones (
  ...
  cantidad_kits smallint not null check (cantidad_kits between 1 and 10),
  kits jsonb not null check (jsonb_typeof(kits) = 'array' and jsonb_array_length(kits) between 1 and 10),
  observaciones text,
