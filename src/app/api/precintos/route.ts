...
export async function POST(request: Request) {
  ...
  try {
    ...
    const data = await createMovimientoKit(supabase, user.id, "recepcion", readMovement(form));
    return NextResponse.json(data);
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : "No fue posible guardar la recepcion." }, { status: 400 });
  }
}
function readMovement(form: FormData) { const file = (key: string) => { const value = form.get(key); return value instanceof File && value.size ? value : null; }; return { embarqueId: String(form.get("embarqueId") ?? ""), observaciones: String(form.get("observaciones") ?? ""), personaUnoNombre: String(form.get("personaUnoNombre") ?? ""), personaUnoCedula: String(form.get("personaUnoCedula") ?? ""), personaDosNombre: String(form.get("personaDosNombre") ?? ""), personaDosCedula: String(form.get("personaDosCedula") ?? ""), foto: file("foto"), firmaUno: file("firmaUno"), firmaDos: file("firmaDos") }; }
