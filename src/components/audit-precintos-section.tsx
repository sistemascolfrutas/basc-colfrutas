...
function TraceabilityDetail({ record, onPreview }: { record: AuditPrecintoRecord; onPreview: (url: string, label: string) => void }) {
  return <section className="space-y-6 rounded-[2rem] border border-slate-200 bg-white/90 p-6 shadow-[0_20px_60px_rgba(15,23,42,0.07)]"><div><span className="text-xs font-bold uppercase tracking-[0.25em] text-emerald-700">Trazabilidad completa</span><h3 className="mt-2 text-2xl font-semibold text-slate-950">Embarque {record.numero_embarque}</h3><p className="mt-2 text-sm text-slate-600">Kit {record.kits[0]?.numero} · Creado {formatDate(record.fecha)} a las {record.hora}</p></div><div className="grid gap-6 lg:grid-cols-2"><MovementCard title="Entrada a Portería" movement={record.entrada} firstLabel="Auxiliar de Comercio" secondLabel="Portería" onPreview={onPreview} /><MovementCard title="Salida a Logística" movement={record.salida} firstLabel="Portería" secondLabel="Logística" onPreview={onPreview} /></div></section>;
}
