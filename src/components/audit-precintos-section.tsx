  ...
  return (
    <section className="space-y-6 border-t border-slate-200 pt-8">
      ...
      <section className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white/90 shadow-[0_20px_60px_rgba(15,23,42,0.07)]">
        ...
        <div className="overflow-x-auto"><table className="w-full min-w-[820px] text-left text-sm"><thead className="bg-slate-50 text-xs font-bold uppercase tracking-[0.15em] text-slate-500"><tr><th className="px-4 py-3">Tipo</th><th className="px-4 py-3">Fecha</th><th className="px-4 py-3">ATEMPI</th><th className="px-4 py-3">COLFRUTAS</th><th className="px-4 py-3">Kits</th><th className="px-4 py-3 text-right">Accion</th></tr></thead>
          <tbody className="divide-y divide-slate-100">{records.map((record) => <tr key={`${record.tipo}-${record.id}`} className={selected?.id === record.id && selected.tipo === record.tipo ? "bg-emerald-50" : "hover:bg-slate-50"}><td className="px-4 py-4"><span className={`rounded-full px-3 py-1 text-xs font-bold uppercase ${record.tipo === "entrada" ? "bg-sky-50 text-sky-700" : "bg-amber-50 text-amber-700"}`}>{record.tipo}</span></td><td className="px-4 py-4">{formatDate(record.fecha)}<span className="block text-xs text-slate-500">{record.hora}</span></td><td className="px-4 py-4 font-medium">{record.empleado_atempi_nombre}</td><td className="px-4 py-4 font-medium">{record.empleado_colfrutas_nombre}</td><td className="px-4 py-4">{record.kits?.map((kit) => kit.numero).join(", ")}</td><td className="px-4 py-4 text-right"><button type="button" onClick={() => setSelected(record)} className="rounded-xl bg-slate-950 px-4 py-2 text-xs font-semibold text-white">Ver detalle</button></td></tr>)}</tbody></table></div>
      </section>
...
function PrecintoDetail({ record, onPreview }: { record: AuditPrecintoRecord; onPreview: (url: string, label: string) => void }) {
  const fields = [["Tipo", record.tipo], ["Fecha", formatDate(record.fecha)], ["Hora inicial", record.hora], ["Hora final", record.hora_final], ["Accion", record.accion], ["Empleado ATEMPI", record.empleado_atempi_nombre], ["Cedula ATEMPI", record.empleado_atempi_cedula], ["Empleado COLFRUTAS", record.empleado_colfrutas_nombre], ["Cedula COLFRUTAS", record.empleado_colfrutas_cedula], ["Cargo COLFRUTAS", record.empleado_colfrutas_cargo], ["Cantidad de kits", String(record.cantidad_kits)], ["Observaciones", record.observaciones || "Sin observaciones"]];
