...
export function Fsu02Form() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const draft = await loadBrowserDraft<Fsu02Input, EvidenciasFsu02Input>(
          DRAFT_KEY,
        );
        ...
        if (active) {
          setErrorMessage("No fue posible recuperar el borrador local de F-SU-02.");
        }
      } finally {
        if (active) {
...
async function loadResponsableOptions() {
  const response = await fetch(`/api/responsables?form=fsu02&t=${Date.now()}`, {
    cache: "no-store",
  });
...
function hasDraftContent(form: Fsu02Input, files: EvidenciasFsu02Input) {
  return (
